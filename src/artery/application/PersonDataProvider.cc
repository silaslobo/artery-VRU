...
const std::map<PersonDataProvider::AngularAcceleration, double> PersonDataProvider::mConfidenceTable {
...
PersonDataProvider::PersonDataProvider(uint32_t id) :
...
void PersonDataProvider::calculateCurvature()
{
    using namespace vanetza::units::si;
    ...
    if (fabs(mPersonKinematics.p_speed) < 1.0 * meter_per_second) {
		// assume straight road below minimum speed
		mCurvature = 0.0 * vanetza::units::reciprocal_metre;
    } else {
        // curvature calculation algorithm
		mCurvature = (mPersonKinematics.p_yaw_rate / radians) / mPersonKinematics.p_speed;
...
void PersonDataProvider::calculateCurvatureConfidence()
{
	...
	AngularAcceleration filter = -mCurvatureConfidenceOutput[1] +
		...
		omega * omega * t_sample * mPersonKinematics.p_yaw_rate -
	...
	mCurvatureConfidenceInput = mPersonKinematics.p_yaw_rate;
	mConfidence = mapOntoConfidence(abs(filter));
}

void PersonDataProvider::update(const PersonKinematics& dynamics)
{
	...
	if (delta > 0.0 * seconds) {
		const auto old_heading = mPersonKinematics.p_heading;
		const auto old_speed = mPersonKinematics.p_speed;
		mPersonKinematics = dynamics;

		if (isnan(mPersonKinematics.p_acceleration)) {
			mPersonKinematics.p_acceleration = (mPersonKinematics.p_speed - old_speed) / delta;
		}

		if (isnan(mPersonKinematics.p_yaw_rate)) {
			auto diff_heading = old_heading - mPersonKinematics.p_heading; // left turn positive
			if (diff_heading > pi * radian) {
				diff_heading -= 2.0 * pi * radians;
			} else if (diff_heading < -pi * radians) {
				diff_heading += 2.0 * pi * radians;
			}
			 mPersonKinematics.p_yaw_rate = diff_heading / delta;
		}
		...
		if (isnan(mPersonKinematics.p_acceleration)) {
			mPersonKinematics.p_acceleration = vanetza::units::Acceleration::from_value(0.0);
		}
		if (isnan(mPersonKinematics.p_yaw_rate)) {
			mPersonKinematics.p_yaw_rate = vanetza::units::AngularVelocity::from_value(0.0);
		}
	} else {
...
double PersonDataProvider::mapOntoConfidence(AngularAcceleration x) const
{
...
void PersonDataProvider::setStationType(StationType type)
{
	mStationType = type;
}

auto PersonDataProvider::getStationType() const -> StationType
{
	return mStationType;
}

void PersonDataProvider::setStationId(uint32_t id)
{
	mStationId = id;
}
