...
public:
    ...
    std::shared_ptr<EnvironmentModelPerson> getPerson(const std::string& objId);

    std::shared_ptr<EnvironmentModelPerson> getPersonFromVehDB(const u_int32_t& vehId);
    ...
    preselectPersons(const std::string& ego, const std::vector<Position>& area);
...
private:
    ...
    bool addPerson(traci::PersonController* person);
    ...
    bool removePerson(const std::string& nodeId);
    ...
    using PersonDB = std::unordered_map<std::string, std::shared_ptr<EnvironmentModelPerson>>;
    using VehiclesDB = std::map<unsigned long, std::shared_ptr<EnvironmentModelPerson>>;

    using ObjectRtreeValue = std::pair<geometry::Box, std::shared_ptr<EnvironmentModelObject>>;
    using PersonRtreeValue = std::pair<geometry::Box, std::shared_ptr<EnvironmentModelPerson>>;

    using ObjectRtree = boost::geometry::index::rtree<ObjectRtreeValue, boost::geometry::index::quadratic<16>>;
    using PersonRtree = boost::geometry::index::rtree<PersonRtreeValue, boost::geometry::index::quadratic<16>>;
