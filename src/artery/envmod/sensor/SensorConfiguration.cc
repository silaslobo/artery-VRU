...
std::vector<Position> createSensorArc(const SensorConfigFov& config, const EnvironmentModelPerson& egoPer)
{
    using boost::units::si::radians;
    static const auto pi = boost::math::constants::pi<double>();
    // heading from vehicle data is headed north (clockwise),
    // OMNeT++ angles are headed east (counter-clockwise)
    const Angle heading = -1.0 * egoPer.getPersonData().heading() - 0.5 * pi * radians;
    Position sensorPos = egoPer.getAttachmentPoint(config.sensorPosition);
    return createSensorArc(config, sensorPos, heading);
}
