...
FovSensor::FovSensor() :
    ...
    mObjectsFigure(nullptr), mObstaclesFigure(nullptr), mPersonsFigure(nullptr)
{
}
...
void FovSensor::measurement()
{
    ...
    auto person = detectPersons();
    mLocalEnvironmentModel->complementObjects(detection, *this);
    mLocalEnvironmentModel->complementObjects(person, *this);
    mLastPersonDetection = std::move(person);
...
SensorDetection FovSensor::detectPersons() const
{
    namespace bg = boost::geometry;
    if (mFovConfig.fieldOfView.range <= 0.0 * boost::units::si::meter) {
        throw std::runtime_error("sensor range is 0 meter or less");
    ...
    SensorDetection personDetection = createSensorCone();
    auto preselPersonsInSensorRange = mGlobalEnvironmentModel->preselectPersons(mFovConfig.egoID, personDetection.sensorCone);
    auto preselObjectsInSensorRange = mGlobalEnvironmentModel->preselectObjects(mFovConfig.egoID, personDetection.sensorCone);

    // get obstacles intersecting with sensor cone
    auto obstacleIntersections = mGlobalEnvironmentModel->preselectObstacles(personDetection.sensorCone);
    ...
    if (mFovConfig.doLineOfSightCheck)
    {
        ...
        // check if objects in sensor cone are hidden by another object or an obstacle
        for (const auto& person : preselPersonsInSensorRange)
        {
            for (const auto& personPoint : person->getOutline())
            {
                // skip objects points outside of sensor cone
                if (!bg::covered_by(personPoint, personDetection.sensorCone)) {
                    continue;
                }

                LineOfSight lineOfSight;
                lineOfSight[0] = personDetection.sensorOrigin;
                lineOfSight[1] = personPoint;

                bool noVehicleOccultation = std::none_of(preselObjectsInSensorRange.begin(), preselObjectsInSensorRange.end(),
                        [&](const std::shared_ptr<EnvironmentModelObject>& person) {
                            return bg::crosses(lineOfSight, person->getOutline());
                        });
                ...
                if (noVehicleOccultation && noObstacleOccultation) {
                    if (personDetection.persons.empty() || personDetection.persons.back() != person) {
                        personDetection.persons.push_back(person);
                    }

                    if (mDrawLinesOfSight) {
                        personDetection.visiblePoints.push_back(personPoint);
                    } else {
                        // no need to check other object points in detail except for visualization
                        break;
                    }
        ...
        personDetection.obstacles.assign(blockingObstacles.begin(), blockingObstacles.end());        
    } else {
        for (const auto& person : preselPersonsInSensorRange) {
            // preselection: object's bounding box and sensor cone's bounding box intersect
            // now: check if their actual geometries intersect somewhere
            if (bg::intersects(person->getOutline(), personDetection.sensorCone)) {
                personDetection.persons.push_back(person);
            }
        }
    }
...
void FovSensor::initializeVisualization()
{
    ...
    bool drawPersons = par("drawDetectedPersons");
    ...
    if (drawPersons && !mPersonsFigure) {
        mPersonsFigure = new cGroupFigure("persons");
        mGroupFigure->addFigure(mPersonsFigure);
    } else if (!drawPersons && mPersonsFigure) {
        delete mPersonsFigure->removeFromParent();
        mPersonsFigure = nullptr;
    }
}
...
void FovSensor::refreshDisplay() const
{
    ...
    if (mLinesOfSightFigure) {
        ...
        const Position& startPersonPoint = mLastPersonDetection->sensorCone.front();

        for (const Position& endPersonPoint : mLastPersonDetection->visiblePoints) {
            auto line = new cLineFigure();
            line->setLineColor(mColor);
            line->setLineStyle(cFigure::LINE_DASHED);
            line->setStart(cFigure::Point { startPersonPoint.x.value(), startPersonPoint.y.value() });
            line->setEnd(cFigure::Point { endPersonPoint.x.value(), endPersonPoint.y.value() });
            mLinesOfSightFigure->addFigure(line);
        }
    }
    ...
    if (mPersonsFigure) {
        // remove previous objects
        while (mPersonsFigure->getNumFigures() > 0) {
            delete mPersonsFigure->removeFigure(0);
        }

        for (const auto& person : mLastPersonDetection->persons) {
            auto polygon = new cPolygonFigure(person->getPersonId().c_str());
            polygon->setFilled(true);
            polygon->setFillColor(mColor);
            polygon->setLineColor(cFigure::RED);
            for (const auto& position : person->getOutline()) {
                polygon->addPoint(cFigure::Point { position.x.value(), position.y.value() });
            }
            mPersonsFigure->addFigure(polygon);
        }
    }
}
