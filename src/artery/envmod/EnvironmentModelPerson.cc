...
EnvironmentModelPerson::EnvironmentModelPerson(const traci::PersonController* person, uint32_t id) :
    PersonDataProvider(id),
    mPersonController(person),
    mLength(person->getLength()),
    mWidth(person->getWidth())
{
    const auto halfWidth = mWidth * 0.5;
    const auto halfLength = mLength * 0.5;
    mRadius = sqrt(halfWidth * halfWidth + halfLength * halfLength);

    setStationType(deriveStationTypeFromVehicleClass(mPersonController->getVehicleClass()));    
    update();
}

void EnvironmentModelPerson::update()
{
    // Update the internal vdp
    PersonDataProvider::update(p_getKinematics(*mPersonController));

    // Recalculate all time and position dependent attributes
    using namespace boost::math::double_constants;
    Angle heading = -1.0 * (getPersonData().heading() - 0.5 * pi * boost::units::si::radian);
    auto affine = transformVehicle(mLength.value(), mWidth.value(), getPersonData().position(), heading);
...
std::string EnvironmentModelPerson::getPersonId() const
{
    return mPersonController->getPersonId();
}

const Position& EnvironmentModelPerson::getAttachmentPoint(const SensorPosition& pos) const
{
...
const PersonDataProvider& EnvironmentModelPerson::getPersonData() const
{
   return *this;
}
} // namespace artery
