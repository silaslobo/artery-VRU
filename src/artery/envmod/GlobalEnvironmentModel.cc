...
void GlobalEnvironmentModel::refresh()
{
    ...
    for (auto& person_kv : mPersons) {
        person_kv.second->update();
    }

    buildPersonRtree();

    if (mDrawPersons) {
        int numPersons = mPersons.size();
        int numFigures = mDrawPersons->getNumFigures();

        // add missing polygon figures
        while (numFigures < numPersons) {
            auto polygon = new cPolygonFigure();
            polygon->setFillColor(cFigure::BLUE);
            polygon->setFilled(true);
            mDrawPersons->addFigure(polygon);
            ++numFigures;
        }
        ...
        while (numFigures > numPersons) {
            --numFigures;
            delete mDrawPersons->removeFigure(numFigures);
        }

        // update figures with current outlines
        int figureIndex = 0;
        for (const auto& person_kv : mPersons) {
            // we add only polygon figures, thus static_cast should be safe
            auto polygon = static_cast<cPolygonFigure*>(mDrawPersons->getFigure(figureIndex));
            std::vector<cFigure::Point> points;
            for (const Position& pos : person_kv.second->getOutline()) {
                points.push_back(cFigure::Point { pos.x.value(), pos.y.value() });
            }
            polygon->setPoints(points);
            ++figureIndex;
...
bool GlobalEnvironmentModel::addPerson(traci::PersonController* person)
{
    uint32_t id = 0;
    if (mIdentityRegistry) {
        auto identity = mIdentityRegistry->lookup<IdentityRegistry::traci>(person->getPersonId());
        if (identity) {
            id = identity->application;
        }
    }

    auto object = std::make_shared<EnvironmentModelPerson>(person, id);
    auto insertion = mPersons.emplace(object->getPersonId(), object);
    if (insertion.second) {
        auto box = boost::geometry::return_envelope<geometry::Box>(object->getOutline());
        mPersonRtree.insert(PersonRtreeValue { std::move(box), object });
    }

    ASSERT(mPersons.size() == mPersonRtree.size());
    return insertion.second;
}
...
void GlobalEnvironmentModel::buildPersonRtree()
{
    struct envelope_maker
    {
        inline PersonRtreeValue operator()(const PersonDB::value_type& obj_kv) const
        {
            const std::shared_ptr<EnvironmentModelPerson>& obj = obj_kv.second;
            auto box = boost::geometry::return_envelope<geometry::Box>(obj->getOutline());
            return PersonRtreeValue { std::move(box), obj };
        }
    };

    // use bulk loading for efficient packing
    mPersonRtree = PersonRtree { mPersons | boost::adaptors::transformed(envelope_maker()) };
    pmTainted = false;
}
...
bool GlobalEnvironmentModel::removePerson(const std::string& personId)
{
    bool erased = mPersons.erase(personId) > 0;
    pmTainted |= erased; /*< pending object rtree update */
    return erased;
}
...
void GlobalEnvironmentModel::removePersons()
{
    mPersons.clear();
    mPersonRtree.clear();
    pmTainted = false;
    ...
    if (mDrawPersons) {
        // remove all polygons
        for (int i = mDrawPersons->getNumFigures() - 1; i >= 0; --i) {
            delete mDrawPersons->removeFigure(i);
        }
    }
}
...
void GlobalEnvironmentModel::initialize()
{
    ...
    if (par("drawPersons")){
        mDrawPersons = new omnetpp::cGroupFigure("persons");
        getCanvas()->addFigure(mDrawPersons);
    }  
    ...
    if (signal == traciNodeAddSignal) {
        ...
        if (module) {
            ...
            if (controller) {
                ...
                auto pController = getPersonController(module);
                std::string elementClass = pController->getVehicleClass();
                if (elementClass == "pedestrian"){
                    addPerson(pController);
                }
            }
        }
...
traci::PersonController* GlobalEnvironmentModel::getPersonController(cModule* module)
{
    assert(module);
    auto person = dynamic_cast<ControllablePerson*>(module->getModuleByPath(par("nodeMobilityModule")));
    return person ? person->getPersonController() : nullptr;
}
...
std::shared_ptr<EnvironmentModelPerson> GlobalEnvironmentModel::getPerson(const std::string& perId)
{
    auto found_Person = mPersons.find(perId);
    return found_Person != mPersons.end() ? found_Person->second : nullptr;
}
...
std::shared_ptr<EnvironmentModelPerson> GlobalEnvironmentModel::getPersonFromVehDB(const u_int32_t& vehId)
{
...
std::vector<std::shared_ptr<EnvironmentModelPerson>>
GlobalEnvironmentModel::preselectPersons(const std::string& ego, const std::vector<Position>& area)
{
    ASSERT(!pmTainted);
    ...
    std::vector<std::shared_ptr<EnvironmentModelPerson>> personsInSearchArea;
    PersonRtree::const_query_iterator it = query_intersections(mPersonRtree, area);
    for (; it != mPersonRtree.qend(); ++it) {
        if (it->second->getPersonId() != ego) {
            personsInSearchArea.push_back(it->second);
        }
    }
    return personsInSearchArea;
}
